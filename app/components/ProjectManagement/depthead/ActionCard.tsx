          ...
          <div className="flex-1 space-y-2">
            ...
            <div className={`flex items-center gap-3 text-xs ${colors.textMuted} flex-wrap`}>
              <div className="flex items-center gap-1">
                ...
                <span>{action.assignedTo?.length || 0} assigned</span>
              </div>
              {action.dueDate && (
                <>
                  <span>•</span>
        ...
        <div className={`relative border-t ${statusColors.border} p-4 space-y-4`} onClick={(e) => e.stopPropagation()}>
          ...
          {action.submissionNote && (
            <div className={`p-3 rounded-lg bg-gradient-to-r ${cardCharacters.informative.bg} border ${cardCharacters.informative.border}`}>
              ...
              <p className={`text-sm ${colors.textPrimary} mb-2 relative z-10`}>{action.submissionNote}</p>
              {action.submittedAt && (
                <p className={`text-xs ${colors.textMuted} relative z-10`}>
                  Submitted by {action.submittedBy || 'Unknown'} on {new Date(action.submittedAt).toLocaleString()}
                </p>
              )}
              {action.submissionAttachments && action.submissionAttachments.length > 0 && (
                <div className="mt-2 flex flex-wrap gap-2 relative z-10">
                  {action.submissionAttachments.map((attachment: string, i: number) => (
                    <a
                      key={i}
